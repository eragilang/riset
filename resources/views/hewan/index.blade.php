            ...
            <div class="mt-5 flex lg:mt-0 lg:ml-4">
                <div class="flex justify-end">
                    <a href="{{ route('hewan.create') }}"
                        class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">+ Tambah Hewan Baru</a>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="border border-gray-300 shadow-sm bg-white rounded-xl filament-tables-container dark:bg-gray-800 dark:border-gray-700">
                <div class="flex justify-between items-center p-4 bg-white dark:bg-gray-900 rounded-xl"">
                        <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="table-search-hewan" class="block p-2 pl-10 w-80 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Cari Hewan">
                        </div>
                    </div>
                <div class="overflow-x-auto relative border-t">
                    <table class="w-full text-left rtl:text-right divide-y table-auto dark:divide-gray-700">
                        ...
                        <tr class="bg-gray-500/5">
                            ...
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">Genre</th>
                        ...
                        @forelse ($hewans as $hewan)
                            <tr class="border-b">
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ $hewan->nama }}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ ucfirst($hewan->genre->genre) }}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">{{ $hewan->status == 0 ? 'Non-Aktif':'Aktif' }}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">{{ $hewan->created_at->format('d-m-Y') }}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">
                                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('hewan.destroy', $hewan->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <a href="{{ route('hewan.show', $hewan->id) }}" id="{{ $hewan->id }}-show-btn"
                                        ...
                                        <a href="{{ route('hewan.edit', $hewan->id) }}" id="{{ $hewan->id }}-edit-btn"
                                            class="inline-block text-white font-medium text-xs leading-tight uppercase rounded-full hover:shadow-lg focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg transition duration-150 ease-in-out">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="black" class="w-6 h-6">
